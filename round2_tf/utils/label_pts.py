...
RED = (0, 0, 255)
def show_prections(img, ptss1,ptss2):
    i = 0
    for coord in ptss1:
        i += 1
...
if __name__=='__main__':
    ...
    catgory_list =['outwear']
    gt_file = '../csv/part_train.csv'
    hard_csv = '../hard.csv'
    pred_csv = '../pred_result/outwear_pred.csv'
    ...
    pred_data = build_data_dict(pred_csv, catgory_list=catgory_list)
    for name in img_list:
        img = cv2.imread(img_dir+name)
        ptss1 = gt_data[name]['joints']
        ptss2 = pred_data[name]['joints']
        #label_one_img(img,ptss)
        show_prections(img, ptss1,ptss2)
